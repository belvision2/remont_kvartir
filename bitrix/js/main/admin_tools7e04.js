...
function JCAdminMenu(sOpenedSections)
{
	...
	this.ToggleDynSection = function(cell, module_id, div_id, level)
	{
		...
		if(div.innerHTML == '')
		{
			...
			this.request.Send('../admin/get_menu415a.html?lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+module_id+'&admin_mnu_menu_id='+encodeURIComponent(div_id));
		}
...
function TabControl(name, unique_name, aTabs)
{
	...
	this.SaveSettings =  function()
	{
		...
		var options_url = '../admin/user_options415a.html?lang='+phpVars.LANGUAGE_ID+'&sessid='+phpVars.bitrix_sessid;
	...
	this.DisableSettings = function()
	{
		...
		request.Send('../admin/user_options415a.html?lang=' + phpVars.LANGUAGE_ID + sParam + '&sessid='+phpVars.bitrix_sessid);
	}
...
function JCUserOptions()
{
	...
	this.SendData = function(callback)
	{
		...
		if(sParam != '')
		{
			...
			_this.request.Send('../admin/user_optionsd41d.html?'+sParam+'&sessid='+phpVars.bitrix_sessid);
		}
	}
	...
	this.DeleteOption = function(sCategory, sName, bCommon, callback)
	{
		...
		_this.request.Action = callback;
		_this.request.Send('../admin/user_options6f46.html?action=delete&amp;c='+sCategory+'&n='+sName+(bCommon == true? '&common=Y':'')+'&sessid='+phpVars.bitrix_sessid);
	}
}
...
function JCStartMenu()
{
	...
	this.ShowStartMenu = function(button, back_url)
	{
		...
		if(!menuStart || !menuStart.menuItems)
		{
			...
			request.Send('../admin/get_start_menu415a.html?lang='+phpVars.LANGUAGE_ID+(back_url? '&back_url_pub='+encodeURIComponent(back_url):'')+'&sessid='+phpVars.bitrix_sessid);
		}
		else
		{
	...
	this.OpenDynMenu = function(menu, module_id, items_id, back_url)
	{
		...
		request.Send('../admin/get_start_menu6888.html?mode=dynamic&amp;lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+encodeURIComponent(module_id)+'&admin_mnu_menu_id='+encodeURIComponent(items_id)+(back_url? '&back_url_pub='+encodeURIComponent(back_url):'')+'&sessid='+phpVars.bitrix_sessid);
	}
	...
	this.OpenURL = function(item, arguments, url, back_url)
	{
		...
		{
			request.Action = function(result){}
			request.Send('../admin/get_start_menucafc.html?mode=save_recent&amp;url='+encodeURIComponent(url)+'&text='+encodeURIComponent(itemInfo['TEXT'])+'&title='+encodeURIComponent(itemInfo['TITLE'])+'&icon='+itemInfo['ICON']+'&sessid='+phpVars.bitrix_sessid);
		}
		if(back_url)
